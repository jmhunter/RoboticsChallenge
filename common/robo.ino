...
void displayChar(char c) {
  ...
  if (c >= 'a' && c <= 'z') c -= 32; // Map to uppercase

  if (c >= 33 && c <= 95) {
    int index = c - 33;
    // Bounds check against font array size
    if (index >= 0 && index < (int)(sizeof(font5x8) / sizeof(font5x8[0]))) {
      // Center the character: (16 - 5) / 2 = 5
      for (int i = 0; i < 5; i++) {
        buffer[i + 5] = pgm_read_byte(&font5x8[index][i]);
      }
    }
  }
  matrix_display(buffer);
}
...
void displayString(const char* s) {
  ...
  int numChars = sizeof(font5x8) / sizeof(font5x8[0]);

  // Show up to 2 characters statically
  for (int charIdx = 0; charIdx < 2 && charIdx < len; charIdx++) {
    char c = s[charIdx];
    if (c >= 'a' && c <= 'z') c -= 32; // Map to uppercase

    if (c >= 33 && c <= 95) {
      int fontIdx = c - 33;
      if (fontIdx >= 0 && fontIdx < numChars) {
        int offset = (charIdx == 0) ? 2 : 9; // 2..6 and 9..13
        for (int i = 0; i < 5; i++) {
          buffer[offset + i] = pgm_read_byte(&font5x8[fontIdx][i]);
        }
      }
    }
  }
...
void displayNumber(int num) {
  displayString(String(num).c_str());
}
